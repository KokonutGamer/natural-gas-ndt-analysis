    ...
    namespace detail
    {
        ...
        public:
            ...
            bool load(handle src, bool convert)
            {
                ...
                // borrow python's reference to the object and reinterpret it into a numpy.ndarray
            ...
            static handle cast(const cv::Mat &m, return_value_policy, handle defval)
            {
                ...
                // strides help numpy determine how many elements to pass to get to the next element along the specified axis
                std::vector<ssize_t> strides = {static_cast<ssize_t>(m.step[0]), static_cast<ssize_t>(m.step[1])};

                // add another element to the shape and stride tuples if multiple color channels exist
                if (m.channels() > 1)
                {
