 ...
 *
 * Processes images using native C++ routines, including morphological
 * operations (dilation, opening, closing), median blurring, and custom
 * histogram-based binary thresholding.
 */
...
public:
   ...
   *
   * Applies maximum and median filters, computes a threshold based on the
   * bottom 5th percentile of the image histogram, applies a binary inverted
   * mask, and performs morphological opening and closing.
   *
   * @param image The input image as an OpenCV matrix.
   * @param method A string identifier for the specific processing algorithm to
   * use (currently unused in the C++ implementation, provided for interface
   * parity).
