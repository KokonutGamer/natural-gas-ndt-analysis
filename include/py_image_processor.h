 ...
 * @brief Concrete implementation of ImageProcessor that delegates execution to Python scripts.
 * 
...
public:
    ...
    /**
     * @brief Dispatches the image processing task to the embedded Python environment.
     * 
     * @param image The input image as an OpenCV matrix.
     * @param method The string identifier of the Python algorithm to use.
     * @return cv::Mat The processed image returned from Python, or an empty cv::Mat if an error occurs.
     */
     ...
     * @return std::string The descriptive name, or "(Get name failed)" if an error occurs.
     */
    std::string getName(const std::string method) const override;

    /**
     * @brief Retrieves a list of all processing methods registered in the Python environment.
     * 
     * @return std::vector<std::string> A vector containing the names of available Python methods.
     */
    std::vector<std::string> getMethods() const;

private:
    /**
     * @brief Scoped interpreter guard that initializes and finalizes the Python interpreter.
     * 
    ...
    /**
     * @brief Pybind11 dictionary containing the registered Python methods/algorithms.
     */
    py::dict registry;
};
