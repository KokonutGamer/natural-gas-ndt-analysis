...
cv::Mat CppImageProcessor::execute(const cv::Mat &image, const std::string method) const {
  ...
  int kernelSize = 3;
  cv::Mat maxKernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(kernelSize, kernelSize),
  ...
  cv::threshold(processedImage, processedImage, thresholdValue, 255.0, cv::ThresholdTypes::THRESH_BINARY_INV);

  // morphological operation parameters
  int numberOfMorphs = 4;
  kernelSize = 5;
  cv::Mat morphKernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(kernelSize, kernelSize),
                                                  cv::Point(kernelSize / 2, kernelSize / 2));

  for (int i = 0; i < numberOfMorphs; i++) {
    // open
    cv::morphologyEx(processedImage, processedImage, cv::MorphTypes::MORPH_OPEN, morphKernel);

    // close
    cv::morphologyEx(processedImage, processedImage, cv::MorphTypes::MORPH_CLOSE, morphKernel);
  }

  return processedImage;
}

std::string CppImageProcessor::getName(const std::string method) const { return "C++ image processor"; }
