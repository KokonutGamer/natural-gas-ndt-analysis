...
Welcome to the developer documentation for the **NDT Analysis Using Image Processing and Computer Vision**. 
...
* `ImageProcessor`: The abstract interface defining the `execute()` and `getName()` methods. All image processing algorithms must inherit from this class.
...
* [MatchedFilterPyProcessor](@ref matched_filter_pyprocessor.MatchedFilterPyProcessor) (Key: `mfccd`): Default implementation of Matched Filter. Processes the image by matching twelve crack templates, taking the maximum response from all templates and discarding small connected components.
* [SmallMFPyProcessor](@ref matched_filter_pyprocessor.SmallMFPyProcessor) (Key: `smf`): Matched filter variation using smaller templates.
* [MFGammaPyProcessor](@ref matched_filter_pyprocessor.MFGammaPyProcessor) (Key: `mfgamma`): Matched filter variation using gamma correction.
* [MFSigmoidPyProcessor](@ref matched_filter_pyprocessor.MFSigmoidPyProcessor) (Key: `mfsigmoid`): Matched filter variation using sigmoid stretching.
* [MFHysteresisPyProcessor](@ref matched_filter_pyprocessor.MFHysteresisPyProcessor) (Key: `mfhyst`): Matched filter variation using hysteresis thresholding.
* [MFHystGammaPyProcessor](@ref matched_filter_pyprocessor.MFHystGammaPyProcessor) (Key: `mfhgamma`): Matched filter variation combining hysteresis thresholding with gamma correction.
* [MFHystSigmoidPyProcessor](@ref matched_filter_pyprocessor.MFHystSigmoidPyProcessor) (Key: `mfhsigmoid`): Matched filter variation combining hysteresis thresholding with sigmoid stretching.
* [PyramidMFPyProcessor](@ref matched_filter_pyprocessor.PyramidMFPyProcessor) (Key: `pmf`): Matched filter variation using a three-level Gaussian pyramid for scale invariance.

### Handling Zero-Copy Memory
A crucial component of this architecture is the `cvmat_caster.hpp`. It defines a custom `pybind11` type caster that maps C++ `cv::Mat` objects directly to Python `numpy.ndarray` objects.

When an image is passed from `PyImageProcessor` to a Python script, it is passed by reference (zero-copy where possible) and modified **in-place** by the Python script, avoiding expensive memory copies between the two languages.
