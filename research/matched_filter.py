...
def apply_matched_filter_downsampled(
    image_path: str,
    ...
    downsampled_img = cv2.resize(
        img_blur, (0, 0), fx=scale, fy=scale, interpolation=cv2.INTER_LINEAR
    )
    ...
    # Scale the binary result back up to the original size using nearest neighbor
    # (to keep binary edges sharp)
    binary_upsampled = cv2.resize(
        binary_crack,
...
def setup_plot(
    image_name: str,
    templates_small: list[np.ndarray],
    ...
    result_downsampled, _ = apply_matched_filter_downsampled(
        image_name, templates_small, scale=0.5
    )
