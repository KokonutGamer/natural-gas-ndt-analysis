...
image_names = ["faint_pit.tiff", "big-pit1.tiff"]


def plot_hist_derivatives(
    image: cv2.typing.MatLike, axs: np.ndarray, row_label: str
) -> None:
    ...
    deriv_sobel = np.gradient(smoothed_hist)

    # savitzky-golay filter (polyorder < window length, first derivative)
    # applies element-wise convolution
    deriv_savgol = savgol_filter(smoothed_hist, window_length=15, polyorder=3, deriv=1)

    # 1D gaussian derivative (sigma for smoothing radius, first derivative)
    deriv_gaussian = gaussian_filter1d(smoothed_hist, sigma=3, order=1)

    # univariate spline derivative (smoothing factor)
    spline = UnivariateSpline(x_bins, smoothed_hist, s=10000)
    ...
    # sobel
    axs[1].plot(x_bins, smoothed_hist, color="gray", alpha=0.5, label="Smoothed Hist")
    ax1_twin = axs[1].twinx()
    ax1_twin.plot(x_bins, deriv_sobel, color="yellow", label="Sobel Deriv")
    axs[1].set_title(f"{row_label}: Sobel 1D")

    # savitzky-golay
    axs[2].plot(x_bins, smoothed_hist, color="gray", alpha=0.5)
    ...
    # gaussian
    axs[3].plot(x_bins, smoothed_hist, color="gray", alpha=0.5)
    ...
    # spline derivative
    axs[4].plot(x_bins, smoothed_hist, color="gray", alpha=0.5)
...
def setup_plot(image_name: str) -> None:
    ...
    hs = min(h, w) // 2
    cropped = image[
        cy - hs : cy + hs, cx - hs : cx + hs
    ]  # cropped to min(h, w)^2, keeping center the same
    ...
    plt.tight_layout()
    fig.savefig(f"figures/{image_name[:ext]}_Smoothed_Deriv.png")
