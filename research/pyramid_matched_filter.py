...
def apply_matched_filter_pyramid(image_path: str, templates: list[np.ndarray], levels: int = 3, threshold: float = 0.50):
    """
    Applies the matched filter across a Gaussian pyramid to detect cracks of varying widths.
    Returns the binary image, global max response, and a list of intermediate step dictionaries for visualization.
    """
    ...
    for level in range(levels):
        ...
        if level > 0:
            level_max_upsampled = cv2.resize(level_max, (original_shape[1], original_shape[0]), 
...
def setup_plot(image_name: str, templates: list[np.ndarray]) -> None:
    ...
    result_img, global_max, step_history = apply_matched_filter_pyramid(image_name, templates, levels=3)
    ...
    if result_img is not None and global_max is not None and step_history is not None:
        ...
        # Determine the number of rows needed (1 for each level + 1 for the final results)
